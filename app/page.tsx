...
function QuotePanel({
  ...
  return (
    <div className="no-print flex flex-col gap-3">
      ...
      <SendOrder
        label="Share the quotation on WhatsApp"
        sub={`${items.length} item${items.length > 1 ? "s" : ""} · ${inr(grandPayable)}`}
 ...
 */
function SendOrder({ label, sub, filename, text, build, disabled }: {
  label: string; sub: string; filename: string; text: string; build: () => Blob | Promise<Blob>; disabled?: boolean;
}) {
  ...
  const go = async () => {
    ...
    try {
      ...
      setState(await sharePdfToWhatsApp(blob, filename, text));
    } catch {
      setState("idle");
    }
  };
  ...
  return (
    <div className="no-print">
      <button onClick={go} disabled={disabled || state === "working"}
